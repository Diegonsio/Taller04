...
public class Ventana extends JFrame{
    ...
    private JPanel panelTrabajadores() {
        ...
        for (int i=0;i<gestorDatos.getPersonas().size();i++) {
            JPanel temp = new JPanel(new GridLayout(1, 10));
            temp.setPreferredSize(new Dimension(1000,50));
            JLabel nombre = new JLabel(gestorDatos.getPersonas().get(i).getNombre());
            temp.add(nombre);
            JLabel apellido = new JLabel(gestorDatos.getPersonas().get(i).getApellido());
            temp.add(apellido);
